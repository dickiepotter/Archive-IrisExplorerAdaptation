...
const char GloveInteractor::geomBuffer[] =  
	{
...
SO_KIT_SOURCE(GloveInteractor);

void GloveInteractor::initClass()
{ 
	SO_KIT_INIT_CLASS(GloveInteractor, SoDragger, "Dragger") ;
}

GloveInteractor::GloveInteractor()
{
	SO_KIT_CONSTRUCTOR(GloveInteractor);
	...
	myMat->diffuseColor.setValue(0.0f,0.0f,1.0f) ;
	myMat = SO_GET_ANY_PART(this, "intNameMat", SoMaterial) ;
	myMat->diffuseColor.setValue(1.0f,1.0f,1.0f) ;
	SoCoordinate3 *myCoords = SO_GET_ANY_PART(this, "axesPoints", SoCoordinate3) ;
	myCoords->point.set1Value(0, 0.0f,0.0f,0.0f) ;
	myCoords->point.set1Value(1, 0.0f,0.0f,0.0f) ;
	...
	axestrans->translation.setValue(.0f, .0f, .0f) ;
	...
	/* ************************************************/
	/*	TIMER SETUP NEEDED FOR POLLING GLOVE		  */
	/**************************************************/
	// Console::WriteLine("Timer SETUP");
	glovepol = new SoTimerSensor(&GloveInteractor::TimerSensorCB, this); 
	//glovepol->setFunction(&GloveInteractor::TimerSensorCB);
	glovepol->setInterval(0.1f);
	

	/* ************************************************/
	/*	MOUSE CALLBACKS NOT NEEDED FOR GLOVE VERSION  */
	/**************************************************/
	//addStartCallback( &GloveInteractor::startCB );
	//addMotionCallback( &GloveInteractor::motionCB );
	//addFinishCallback( &GloveInteractor::finishCB );
	/****************************************************/

	// Updates the translation field when the motionMatrix is set.
	addValueChangedCallback( &GloveInteractor::valueChangedCB );
	addOtherEventCallback(&GloveInteractor::metaKeyChangeCB) ;
	
	// Updates the motionMatrix when the translation field is set.
	fieldSensor = new SoFieldSensor( &GloveInteractor::fieldSensorCB, this);
...
GloveInteractor::~GloveInteractor()
{
	delete planeProj;
	glove->P5_Close();
	if (glove) delete glove;
	if (fieldSensor) delete fieldSensor;
}


void GloveInteractor::metaKeyChangeCB(void *, SoDragger *inDragger)
{
	GloveInteractor *dragger = (GloveInteractor *) inDragger ;
...
SbBool GloveInteractor::setUpConnections( SbBool onOff, SbBool doItAlways )
{
...
void GloveInteractor::dragStart()
{
...
void GloveInteractor::checkLimits(void)
{
...
void GloveInteractor::printInformation(void) 
{
...
void GloveInteractor::setTheName(char* name) 
{
	SoText2 *myText = SO_GET_ANY_PART(this, "intNameText", SoText2) ;
	myText->string = name ;
}

void GloveInteractor::updateText(void) 
{
	char buffer[50] ;
...
void GloveInteractor::updateAxes(void)
{
...
void GloveInteractor::dragFinish()
{
	setSwitchValue( translatorSwitch.getValue(), 0 );
	setSwitchValue( feedbackSwitch.getValue(), 0 );
	printInformation() ;
...
void GloveInteractor::startCB( void *, SoDragger *inDragger )
{
	GloveInteractor *m = (GloveInteractor *) inDragger;
	m->dragStart();
}

void GloveInteractor::motionCB( void *, SoDragger *inDragger )
{
	GloveInteractor *m = (GloveInteractor *) inDragger;
	m->drag();
}

void GloveInteractor::finishCB( void *, SoDragger *inDragger )
{
	GloveInteractor *m = (GloveInteractor *) inDragger;
	m->dragFinish();
}

void GloveInteractor::valueChangedCB( void *, SoDragger *inDragger )
{
	GloveInteractor *m = (GloveInteractor *) inDragger;
...
void GloveInteractor::fieldSensorCB( void *inDragger, SoSensor *)
{
	GloveInteractor *dragger = (GloveInteractor *) inDragger;
...
void GloveInteractor::TimerSensorCB( void *inDragger, SoSensor *)
{
	GloveInteractor *dragger = (GloveInteractor *) inDragger;
	SoTranslation *myPart = SO_GET_ANY_PART(dragger, "translatorTranslate", SoTranslation) ;

	if(dragger->glove->m_nNumP5 > 0)
	{
		SbVec3f vec = myPart->translation.getValue()
		float x				= dragger->glove->m_P5Devices[0].m_fx;
		float y				= dragger->glove->m_P5Devices[0].m_fy;
		float z				= dragger->glove->m_P5Devices[0].m_fz;
		/*float *rotation		= (float*)dragger->glove->m_P5Devices[0].m_fRotMat;
		...
		SbMatrix moveMatrix
		(
			dragger->glove->m_P5Devices[0].m_fRotMat[0][0]	+x,	
			dragger->glove->m_P5Devices[0].m_fRotMat[0][1],		
			dragger->glove->m_P5Devices[0].m_fRotMat[0][2],		
			0,
			dragger->glove->m_P5Devices[0].m_fRotMat[1][0],		
			dragger->glove->m_P5Devices[0].m_fRotMat[1][1]	+y,	
			dragger->glove->m_P5Devices[0].m_fRotMat[1][2],		
			0,
			dragger->glove->m_P5Devices[0].m_fRotMat[2][0],		
			dragger->glove->m_P5Devices[0].m_fRotMat[2][1],		
			dragger->glove->m_P5Devices[0].m_fRotMat[2][2]	+z,	
			0,
			0,0,0,1
		);*/

		myPart->translation.setValue(x,y,z) ;
		printf("x=%d y=%d z=%d", x,y,z);

		//dragger->setMotionMatrix(moveMatrix);
		//dragger->printInformation() ;
	}
}
