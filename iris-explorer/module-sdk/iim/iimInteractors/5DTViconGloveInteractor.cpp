#include "stdafx.h"
#include "5DTViconGloveInteractor.h"
...
GloveInteractor::GloveInteractor()
{
	...
	glovepol = new SoTimerSensor(); 
	glovepol->setFunction(&Interactor::TimerSensorCB);
	glovepol->setInterval(0.01f);
	*/
	...
	pGlove = fdOpen( "COM1" );
	if( pGlove != NULL ) cout << "Glove found" << endl;
	else cout << "Glove NOT found" << endl;

	query = KTrackerQuery::Instance();
	track = query->CreateTracker("vicon");
	if( track != NULL && track->Open("net:tracker") ) cout << "Tracker found" << endl;
	else cout << "Tracker NOT found" << endl;

	for (int n=1; n<=track->Ports(); n++) 
		track->Enable(n);
...
/*void GloveInteractor::TimerSensorCB( void *, SoSensor *)
{
	cout << "Polling" << endl;
	if( fdGetGesture(pGlove) == 0 )
	{
		track->UpdateNow();
		for (int n=0; n<track->Count(); n++)
		{
			const KTrackerData &item = track->Item(n);
			SbMatrix gloveMat(item.P().x,0,0,0,0,item.P().y,0,0,0,0,item.P().z,0,0,0,0,0);
			
			setMotionMatrix(gloveMat);
			// o << item.P().x << " " << item.P().y << " " << item.P().z;
		}
	}
}*/