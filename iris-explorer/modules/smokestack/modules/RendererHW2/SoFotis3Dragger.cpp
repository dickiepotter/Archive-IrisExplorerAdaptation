...
//	printf("Open Inventor version: %s\n",SoDB::getVersion()) ;
    ...
    SO_KIT_ADD_FIELD(once, (1));       // HW
    SO_KIT_ADD_FIELD(mode, (0));       // HW
...
//       myPart->translation.setValue(currentValue[0],myPart->translation.getValue()[1] ,0.0) ;
    ...
    if ( once.getValue()) {   // if this is a movable interactor, cue out-of-sync
		                      // unless in reset mode
		SoMaterial *mat = SO_GET_ANY_PART(this, "axesXTextMat", SoMaterial) ;
		if (mode.getValue() != 0) mat->diffuseColor.setValue(0.0, 0.0, 0.0) ;
...
////////////////////////////////////////////////////////////////////////
{
    ...
    if ( once.getValue()) {   
       printInformation();
       if (mode.getValue() == 2) { 
			once.setValue(0); // make one-shot sender in run mode
			// and constrain dragger as it is  - HW
			minimum.setValue(value.getValue()[0],value.getValue()[1],value.getValue()[2]) ;
			maximum.setValue(value.getValue()[0],value.getValue()[1],value.getValue()[2]) ;
	   }
    }

    // visually cue is disabled (red) or out-of-sync (black)
    SoMaterial *mat = SO_GET_ANY_PART(this, "axesXTextMat", SoMaterial) ;
    if (mode.getValue() == 1) mat->diffuseColor.setValue(0.0, 0.0, 0.0) ;
    if (mode.getValue() == 2) mat->diffuseColor.setValue(1.0, 0.0, 0.0) ;
